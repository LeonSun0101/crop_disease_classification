...
class BaseTrainer:
    ...
    def train(self):
        ...
        for cur_epoch in range(1, total_epoch_num+1):
            ...
            if self.cur_epoch==1:
                self.model.save_train_eval_best(self.cur_epoch)
                self.model.save_val_eval_best(self.cur_epoch)
            else:
                print("self.cur_epoch",self.cur_epoch)
                if (self.eval_train>global_value._get_train_max()):
                    self.model.save_train_eval_best(self.cur_epoch)
                    global _train_eval_list
                if (self.eval_validate>global_value._get_val_max()):
                    self.model.save_val_eval_best(self.cur_epoch)
                    global _val_eval_list
            global_value._set_train_eval_value(self.eval_train)
            global_value._set_val_eval_value(self.eval_validate)
            # tensorboard summary
