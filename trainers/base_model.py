...
class BaseModel:
    ...
    def save_check_point(self,epoch):
        ...
        save_name = os.path.join(save_path,'eooch'+str(epoch)+self.config['save_name'])
    ...
    def save_train_eval_best(self,epoch):
        """
        implement the logic of saving model
        """
        print("Saving model...")
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        save_name = os.path.join(save_path,self.config['train_eval_best'])
        state_dict = OrderedDict()
        for item, value in self.net.state_dict().items():
            if 'module' in item.split('.')[0]:
                name = '.'.join(item.split('.')[1:])
        ...
        torch.save(state_dict, save_name)
        print("Model saved: ", save_name)
    def save_val_eval_best(self,epoch):
        """
        implement the logic of saving model
        """
        print("Saving model...")
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        save_name = os.path.join(save_path,self.config['val_eval_best'])
        state_dict = OrderedDict()
        for item, value in self.net.state_dict().items():
            if 'module' in item.split('.')[0]:
                name = '.'.join(item.split('.')[1:])
